	...
	for char_code in char_codes {
		...
		for row in 0..height {
			...
			for col in 0..width {
				...
				// Clamp the signed distance to the spread and normalize it to an i8
...
fn find_dist_to_line(p: &Vector, s: &Vector, e: &Vector) -> f64 {
	...
	// Find the minimal distance to the line segment
...
fn find_dist_to_bezier(p: &Vector, s: &Vector, c: &Vector, e: &Vector) -> f64 {
	...
	for root in depressed_roots {
		// Map the depressed root to the original cubic polynomial
		let raw_t = root - d3 / d4;
